...
async function kitIfCompiler(bin, pr) {
    ...
    if (gcc_res) {
        ...
        if (isWin32 && bin.toLowerCase().includes('mingw')) {
            ...
            if (await pr_1.fs.exists(mingwMakePath)) {
 ...
 */
async function collectDevBatVars(devbat, args, major_version, common_dir) {
    ...
    const batpath = path.join(paths_1.default.tmpDir, batfname);
    const envpath = path.join(paths_1.default.tmpDir, envfname);
    try {
    ...
    try {
        ...
        env = await pr_1.fs.readFile(envpath, { encoding: "utf8" });
        await pr_1.fs.unlink(envpath);
    }
    catch (error) {
        log.error(error);
